...
- ⚠️ **THE PIPE TRAP, SECOND INSTANCE, 2026-08-15: `xcodebuild` EXITED 0 HAVING BUILT
  NOTHING.** The invocation named a `-destination` that did not exist, so the build never ran;
  the command was piped to `tail`, and **`tail` succeeded at printing the complaint**, so the
  status was `tail`'s. This is the same shape as the `osxkeychain` push (`docs/WORKING-DISCIPLINE.md`)
  — the first instance cost a push that had not happened, this one cost a build that had not
  happened — and it recurred *because the first was recorded as an incident about `git push`
  rather than as a rule about pipelines*. Hence the rule in `CLAUDE.md` is written about the
  shell, not about either tool.

  **The bash reflex is worse than nothing here, and this was measured on this box rather than
  recalled.** In zsh, `$PIPESTATUS` is **empty** — it is a bash array — so the idiomatic
  `${PIPESTATUS[0]}` guard lifted from bash evaluates to nothing, passes, and **guards nothing
  while reading exactly like a guard**, which is member 1 of the family (a check whose subject is
  empty by accident). zsh's own spelling is `$pipestatus`, lowercase, and it is **clobbered by
  the very next command** — the first attempt to measure it here printed `[0]` because an
  intervening `echo` had already reset it, so even the correct spelling produces a false green
  one line later. Measured, in zsh: `false | true` → `pipestatus=[1 0]`, `PIPESTATUS=[]`; after
  one `echo`, `pipestatus=[0]`. **`set -o pipefail` is the fix and it works in both shells**
  (measured: `false | tail -1` → exit 1 with it, exit 0 without), because it needs no variable to
  survive to the next line. Better still where it is practical: do not pipe — redirect to a log
  and read the log.

  ⚠️ **AND `pipefail` ONLY FIXES THE STATUS. IT DOES NOT MAKE A GREEN STATUS MEAN THE WORK
  HAPPENED.** A build can legitimately exit 0 having compiled nothing, so the durable half is to
  **assert on the artifact**: resolve the destination from `xcodebuild -showdestinations` and
  match the name exactly rather than typing one, then check the `.app` bundle exists and its
  mtime is newer than the run. **The exit code is a claim about the command; the artifact is the
  subject.**
