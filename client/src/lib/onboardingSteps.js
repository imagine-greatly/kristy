// Kristy's onboarding flow — a short, conversational profile setup.
// Each step drives one screen in <Onboarding/>. The collected answers are
// turned into the payload POSTed to /api/onboarding/full, where the server
// computes TDEE-based macro goals.
...
// Kristy's closing lines once onboarding finishes. `quick` runs when someone
// only gave their name (just-tracking path); `full` runs after the macro card
// appears. Direct, no affirmations — ready for the UI to render on completion.
