...
3. If a word or line is illegible, do NOT guess it. Return that entry with "unreadable": true and put whatever letters you could actually make out in "text" (an empty string if none). A guess is worse than a blank here — someone will shop from this.
   ...
   AN UNREADABLE LINE IS STILL A LINE. RETURN IT — do not leave it out. There is a written item there and the shopper needs to see that you saw something, so they can tell you what it says. Silently omitting it loses an item off their list with no trace, which they will only discover in the store. A line you cannot read gets an entry with "unreadable": true; the ONLY lines that get omitted entirely are crossed-out ones and things that were never shopping items (rule 4 and rule 5). Blurred, smudged, faded, water-damaged, cut off by the edge of the photo: all of those are unreadable ENTRIES, not absences.
